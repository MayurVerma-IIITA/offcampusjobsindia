...
export async function generateJobDraft({ careerUrl, jobText }: { careerUrl?: string; jobText?: string }) {
  if (!jobText && !careerUrl) {
    throw new Error("Provide pasted job text or a company career URL.");
  }
  ...
  const careerPageText = await fetchCareerUrl(careerUrl);
...
Career URL: ${careerUrl || "not provided"}
Pasted job text:
...
${careerPageText || "not available"}`;
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      if (!parsed.success) {
        throw new Error("Generated content did not match the expected structure.");
      }
...
export async function POST(request: Request) {
  ...
  try {
    const draft = await generateJobDraft({
      careerUrl: body.data.careerUrl || undefined,
      jobText: body.data.jobText,
    });
    ...
    if (message.includes("403") || message.includes("API key") || message.includes("GEMINI_API_KEY")) {
      return NextResponse.json(
